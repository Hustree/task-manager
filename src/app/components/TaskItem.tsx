...
import { CalendarIcon, Pencil, Trash2, AlertTriangle, AlertCircle, Check, Star } from 'lucide-react';
...
export function TaskItem({ 
  ...
  return (
    <div className="flex items-center gap-3 p-3 hover:bg-gray-800 rounded-lg group">
      ...
      />
      {isEditing ? (
        <form onSubmit={(e) => { e.preventDefault(); handleEdit(); }} className="flex-1">
          <Input
            value={editedTitle}
        ...
        <span className={cn("flex-1 group-hover:text-white", task.completed && "line-through text-gray-400")}>
          {task.title}
        </span>
      )}
      <Button 
      ...
      <Popover>
        ...
        <PopoverContent className="w-40">
          <div className="grid gap-2">
            <Button variant="outline" onClick={() => onUpdatePriority(task.id, 'low')}>
              Low
            </Button>
            <Button variant="outline" onClick={() => onUpdatePriority(task.id, 'medium')}>
              Medium
            </Button>
            <Button variant="outline" onClick={() => onUpdatePriority(task.id, 'high')}>
              High
            </Button>
          </div>
        </PopoverContent>
